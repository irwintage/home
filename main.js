...
async function loadShader(url){
  ...
  if(!response.ok){
    throw new Error(
      `Shader load failed: ${url} (${response.status})`
    );
  }
...
const [
  ...
  loadShader('./shaders/terrain.vert'),
  loadShader('./shaders/terrain.frag'),

  loadShader('./shaders/final-bg.vert'),
  loadShader('./shaders/final-bg.frag')
...
console.log(
  '✓ External terrain shaders loaded'
);
...
if(
  ...
  mat.customProgramCacheKey = () =>
    'chrysasynth-terrain-external-v1';
...
function updateUI(t){
  ...
  const openingPulse =
    ...
    if(
  terrainShader &&
  terrainShader.uniforms.uOpening
){

  terrainShader.uniforms.uOpening.value =
    openingPulse;
...
if(
  ...
  finalBgMaterial.uniforms.uIntensity.value =
    abstract;
...
function animate(){
 ...
 if(
  terrainShader &&
  terrainShader.uniforms.uTime
){

  terrainShader.uniforms.uTime.value +=
    delta;
